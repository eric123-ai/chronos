      ...
      <div className="mx-auto max-w-7xl px-4 py-6">
        ...
        <div className="mt-4 grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
          ...
          <section className="glass-surface rounded-[32px] p-5">
            ...
            <div className="mt-4 space-y-3">{historyLog.length ? historyLog.slice(0, 8).map((record) => <button key={record.date} type="button" onClick={() => setSelectedDate(record.date)} className="w-full rounded-3xl border border-[rgba(45,35,25,0.08)] bg-[rgba(255,251,245,0.96)] px-4 py-4 text-left transition hover:bg-[rgba(247,241,232,0.96)]"><div className="flex items-center justify-between gap-3"><div className="text-sm font-semibold text-[var(--vf-text)]">{record.date}</div><div className="text-xs text-[var(--vf-text-muted)]">+{record.totalPoints} / -{record.spentPoints ?? 0}</div></div><div className="mt-2 text-xs text-[var(--vf-text-soft)]">{record.completedTasks.length} tasks</div></button>) : <div className="rounded-3xl border border-[rgba(45,35,25,0.08)] bg-[rgba(255,251,245,0.96)] px-4 py-6 text-sm text-[var(--vf-text-muted)]">{text.noHistory}</div>}</div>
          </section>
        </div>
